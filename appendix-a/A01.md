---
title: "A01. Claude Code 사용법"
nav_order: 1
parent: 부록 A
---


Claude Code를 처음 접하는 분들을 위한 실전 가이드입니다. Claude Code의 기본 개념과 구조는 24장에서 설명했습니다. 여기서는 설치부터 실전 활용까지 다룹니다.
...
서버 개발자의 관점에서 Claude Code를 실무에 활용하는 방법을 공유합니다.

### 영어로 프롬프트 쓰기

한국어와 영어는 토큰 소모량이 약 3배 차이가 납니다:
...
영어로 프롬프트를 작성하면 비용 절감 외에도 미묘한 차이가 있습니다. 한국어로 시켰을 때 "이건 못해요"라고 하던 것들이 영어로 하면 되는 경우도 있습니다.

### 백그라운드 터미널 자동 디버깅

Copilot이나 Cursor와 Claude Code의 가장 큰 차이점은 터미널에 대한 읽기/쓰기 권한입니다.
...
Claude가 백그라운드에서 로그를 계속 보고 있기 때문에, 직접 로그를 확인하지 않아도 됩니다. Syntax Error나 Runtime Error가 발생하면 Claude가 자동으로 감지해서 수정합니다.

### 로컬-리모트 배포 자동화

백엔드 개발자는 로컬에서 개발한 후 리모트 서버(AWS 등)에 배포해야 합니다. Claude Code에게 서버 접속 정보를 직접 주고 배포까지 자동화할 수 있습니다:
...
리모트 서버에서 발생하는 문제점을 Claude가 감지하고, 로컬에서 수정해서 다시 Push하는 것까지 자동으로 됩니다.
