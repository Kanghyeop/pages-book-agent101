...
![에이전트 기본 구조](assets/agent-structure.svg)
...
![지능적 에이전트의 4가지 조건](assets/agent-conditions.svg)
...
flowchart TB
    subgraph NotAgent["에이전트라고 불리지만 아닌 것들"]
        A["RAG<br/>질문 → 검색 → 답변<br/>✗ 일회성, 자신의 의제 없음"]
        B["프롬프트 체이닝<br/>요약 → 번역 → 감정분석<br/>✗ 미리 정해진 경로, 분기 없음"]
        C["함수 호출<br/>LLM → API 호출 → 결과<br/>✗ 단순히 도구를 쓸 수 있는 능력일 뿐"]
    end
...
![에이전트 스펙트럼](assets/agent-spectrum.svg)
